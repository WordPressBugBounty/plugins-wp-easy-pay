...
document.addEventListener('DOMContentLoaded', async function () {
	...
	function validateRecaptcha(current_form_id) {
		// NEW: Check for new captcha provider system first
		var captcha_provider = wpep_local_vars.captcha_provider || 'recaptcha';

		// If none is selected, skip validation
		if (captcha_provider === 'none' || !captcha_provider) {
			return true;
		}

		// reCAPTCHA validation
		if (captcha_provider === 'recaptcha') {
			// Backward compatibility: also check old enable_recaptcha option
			if (wpep_local_vars.enable_recaptcha === 'on' || captcha_provider === 'recaptcha') {
				var recaptcha_val = (wpep_local_vars.recaptcha_version === 'v2')
					? (typeof grecaptcha !== 'undefined' ? grecaptcha.getResponse() : '')
					: jQuery('#g-recaptcha-response').val();

				if (!recaptcha_val || recaptcha_val.length === 0) {
				...
				recaptcha = recaptcha_val; // Update global variable
		...
		if (captcha_provider === 'hcaptcha') {
			...
			if (typeof hcaptcha !== 'undefined') {
				hcaptcha_response = hcaptcha.getResponse();
			} else {
				// Try to get from hidden input if available
				hcaptcha_response = jQuery('textarea[name="h-captcha-response"]').val() || '';
			}

			if (!hcaptcha_response || hcaptcha_response.length === 0) {
				jQuery("#theForm-" + current_form_id + " .paymentsBlocks-" + current_form_id)
					.prepend('<div class="wpep-alert wpep-alert-danger wpep-alert-dismissable"><a href="#" data-dismiss="alert" class="wpep-alert-close">×</a>Please verify that you are not a robot.</div>');
				jQuery('.wpepLoader').remove();
				return false;
			}
			recaptcha = hcaptcha_response; // Update global variable
			return hcaptcha_response;
		}

		// Turnstile validation - use hidden input only (Cloudflare auto-fills cf-turnstile-response)
		if (captcha_provider === 'turnstile') {
			var turnstile_response = jQuery("#theForm-" + current_form_id).find('input[name="cf-turnstile-response"]').val() ||
				jQuery('input[name="cf-turnstile-response"]').val() || '';

			if (!turnstile_response || turnstile_response.length === 0) {
				jQuery("#theForm-" + current_form_id + " .paymentsBlocks-" + current_form_id)
					.prepend('<div class="wpep-alert wpep-alert-danger wpep-alert-dismissable"><a href="#" data-dismiss="alert" class="wpep-alert-close">×</a>Please verify that you are not a robot.</div>');
				jQuery('.wpepLoader').remove();
				return false;
			}
			recaptcha = turnstile_response; // Update global variable
			return true; // Return true for success
		}

		// OLD: Backward compatibility for old enable_recaptcha option
	...
	async function createPayment(token, current_form_id, currency, cof, verifyToken = false, recaptcha) {
				...
				success: function (response) {
					...
					if (square_pro_validation.status == 'success') {
						if (undefined !== jQuery('#wpep_file_upload_field')[0]) {
							var files = jQuery('#wpep_file_upload_field')[0].files;
								...
								jQuery.ajax({
									...
									success: function (response) {
										...
										if (parsed_response.hasOwnProperty('error')) {
											...
											var uploadedUrls = parsed_response.uploaded_file_urls || [];
											if (uploadedUrls.length === 0 && parsed_response.uploaded_file_url) {
												uploadedUrls = [parsed_response.uploaded_file_url];
											}
											wpep_file_upload_url(uploadedUrls);
											saveFileUploadUrlToSession(uploadedUrls, data, current_form_id);
										}
									}
								});
							} else {
		...
		function wpep_file_upload_url(urls) {
			var uploadedUrlValue = urls;
			if (Array.isArray(urls)) {
				uploadedUrlValue = urls.join(', ');
			}
			form_values.push(
				{
					label: 'Uploaded URL',
					value: uploadedUrlValue
				}
			);
		}
	...
	function paymentButtonClicked(event, card, current_form_id, currency) {
		...
		jQuery("#theForm-" + current_form_id + " .wizard-fieldset.show .fieldMainWrapper div.wpep-required").each(
			function () {
				...
				if (jQuery('#theForm-' + current_form_id).find('input[type="file"]').length) {
					...
					let isFileRequired = fileInput.prop('required');
					let selectedFiles = fileInput[0] && fileInput[0].files ? fileInput[0].files.length : 0;

					if (isFileRequired && selectedFiles === 0) {
						if (fileInput.siblings('.wpepError').length === 0) {
							jQuery(wpepError).insertAfter(fileInput);
						}
						jQuery(fileInput.siblings('.wpepError')).text('Required Field');
						result2 = false;
					}

					if (selectedFiles > 0 && fileVal !== '') {
		...
		// Hard stop guard: never allow submit when a required file input is empty.
		var requiredFileMissing = false;
		jQuery('#theForm-' + current_form_id).find('input[type="file"][required]').each(function () {
			var requiredFileInput = jQuery(this);
			var hasSelectedFile = this.files && this.files.length > 0;
			if (!hasSelectedFile) {
				if (requiredFileInput.siblings('.wpepError').length === 0) {
					jQuery('<span />').attr('class', 'wpepError').html('Required Field').insertAfter(requiredFileInput);
				} else {
					requiredFileInput.siblings('.wpepError').text('Required Field');
				}
				requiredFileMissing = true;
			} else {
				requiredFileInput.siblings('.wpepError').remove();
			}
		});
...
async function initializeGooglepay(payments, current_form_id, currency, googlePay) {
	...
	if ((parsedVal > 0 && parsedVal >= min && parsedVal <= max) || (!isNaN(parsedAmount) && parsedAmount > 0 && parsedAmount >= min && parsedAmount <= max) && wpep_local_vars.wpep_square_user_defined_amount == 'on') {
		...
		await gPay.attach('#google-pay-button-' + current_form_id);
		jQuery('#gpay-amount').hide();

		return gPay;
	} else if (!isNaN(parsedAmount) && parsedAmount > 0 && wpep_local_vars.wpep_square_user_defined_amount != 'on') {
		const paymentRequest = gpaybuildPaymentRequest(payments, current_form_id, currency, googlePay);
		const gPay = await payments.googlePay(paymentRequest);
		await gPay.attach('#google-pay-button-' + current_form_id);
		jQuery('#gpay-amount').hide();
		return gPay;
	} else if (!isNaN(parsedAmount) && parsedAmount > 0 && (wpep_local_vars.wpep_square_amount_type == 'payment_radio' || wpep_local_vars.wpep_square_amount_type == 'payment_drop' || wpep_local_vars.wpep_square_amount_type == 'payment_tabular' || wpep_local_vars.wpep_square_amount_type == 'tabular_layout_for_square')) {
		const paymentRequest = gpaybuildPaymentRequest(payments, current_form_id, currency, googlePay);
		const gPay = await payments.googlePay(paymentRequest);
		await gPay.attach('#google-pay-button-' + current_form_id);
		jQuery('#gpay-amount').hide();
