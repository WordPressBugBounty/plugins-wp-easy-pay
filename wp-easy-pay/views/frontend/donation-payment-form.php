 ...
 *
 * This file is responsible for rendering the payment form and handling its various components,
 * including field types, conditional rendering, and dynamic values. It integrates with
 * Square payment and authorization modules to process payments through the WP Easy Pay plugin.
 *
 * @package WP_Easy_Pay
 */
	...
	<div class="btnGroup ifSingle">
		...
		<div style="display:flex">
				...
				<div class="wpep-payment-details-wrapper">
					...
					<ul>
						...
						<li class="wpep-fee-onetime">
							...
							<?php if ( 'symbol' === $currency_symbol_type ) { ?>
								<span class="fee_value"><?php echo esc_attr( $square_currency ) . esc_attr( number_format( $wpep_signup_fees_value, 2 ) ); ?></span>
							<?php } elseif ( 'code' === $currency_symbol_type ) { ?>
								<span class="fee_value"><?php echo esc_attr( number_format( $wpep_signup_fees_value, 2 ) ) . ' ' . esc_attr( $square_currency ); ?></span>
							<?php } else { ?>
								<span class="fee_value"><?php echo esc_attr( number_format( $wpep_signup_fees_value, 2 ) ); ?></span>
							<?php } ?>
			...
			if ( 'on' === $wpep_show_wizard ) :
				echo esc_attr( 'wpep-wizard-form-submit-btn wpep-single-form-' . $wpep_current_form_id );
			else :
				echo esc_attr( 'wpep-single-form-submit-btn wpep-single-form-' . $wpep_current_form_id );
			endif;
			?>
			<?php
			...
			?>
				...
				<span>
					<b id="dosign" style="display: none;">$</b><small id="amount_display_<?php echo esc_attr( $wpep_current_form_id ); ?>"
																	class="display">
																	<?php
																	if ( ( ! empty( $fees_data[0]['check'] ) && in_array( 'yes', $fees_data[0]['check'], true ) && isset( $total_amount ) ) || 'yes' === $wpep_enable_signup_fees ) :
																		if ( 'symbol' === $currency_symbol_type ) {
																			echo esc_html( $square_currency . number_format( $total_amount, 2 ) );
																		} elseif ( 'code' === $currency_symbol_type ) {
